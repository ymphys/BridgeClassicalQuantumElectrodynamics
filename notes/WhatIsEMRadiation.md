...
既然能量守恒无法给出令人满意的辐射反作用力，我们只能直接计算粒子的场对自身的作用力，不幸的是经典电动力学中电子被当作点粒子，其产生的场在趋近其自身时发散。如果我们假设电子不是点粒子，而是具有一定的空间分布（形状因子, form factor），则**自作用力(self-force)**的确是可以计算的，具体的结果自然与假设的空间分布有关，但当我们对计算结果取点粒子极限，仍然可以得到一些有意义的结果。一般而言，自作用力的结果具有如下结构：
$$
F_{\text{self}}=\frac{q^2}{4\pi\epsilon_0}\left[-A\frac{a(t)}{c^2\ell}+\frac{2\dot{a}(t)}{3c^3}+\mathcal{O}(\ell)\right]
$$
其中$\ell$是电子形状因子的特征尺度，第一项会贡献到电子的质量，系数$A$与所取的形状因子有关，对Griffith所取的dumbbell 形状因子，第一项恰好对应电子的静电势能[^4]，对Lorentz所取的球形形状因子则会多出一个3/4[^5]. 然而该项在点粒子极限下发散，对应电子的质量发散。有趣的是，第二项与所取形状因子无关，且在点粒子极限下仍然存在，对应的正是Abraham-Lorentz力，这说明它有其正确性，这种正确性正是来自于能量守恒的要求。但是运动方程的三阶时间导数对因果性的破坏仍然是不可接受的，对此经典电动力学是无能为力的。

此外，当假设电子具有空间分布，且仍作为一个刚体运动时，会立即带来违反狭义相对论的后果，原因是刚体要求电子作为整体同时响应外场，这要求电子内部不同位置之间瞬时传递信息。同时，想象一个具有特定空间分布的、非点粒子的电子，由于其自身不同部分之间只存在库伦排斥力，这种结构本身就是不稳定的（不处于能量最低的状态）。因此Poincare在1905年提出必须存在非电磁性质的内应力来维持电子结构的稳定，Schwinger在1983年完善了这种模型[^6]，构建了形式上协变的稳定的电子经典模型，但其增加的内应力没有对应的动力学方程，因此只能被视为为了防止破坏协变性作出的人工修补，是经典电子模型的极限。

### 经典电动力学适用极限

然而，经典电动力学在绝大多数电磁问题上依然是有效的，即使是及到辐射及其反作用，原因是辐射反作用实在是太小了。定量地估算一下，假设一个电子在时间$T$内具有加速度$a$, 其辐射的能量大约为
$$
E_{\text{rad}}=P_{\text{rad}}T\sim\frac{2Q^2 a^2 T}{3 c^3}
$$
而电子在这段时间内获得的动能大约是$ma^2T^2$量级，只要$E_{\text{rad}}\ll ma^2T^2$, 即$T\gg 2Q^2/(3mc^3)=6.26\times10^{-24}$s. 辐射反作用力就可以在短时间运动内被忽略，而长时间积累的效应就可以通过AL公式进行描述。

下面举一些说明辐射反作用不能被忽略的情况：

1. 同步辐射装置的稳定运行要求考虑到电子的辐射反作用，并通过射频电场补充电子的能量，以维持同步光源稳定输出。
2. 超强激光与物质相互作用，当激光功率密度达到$10^{15}\text{W}/\text{cm}^2$, 电子在一个激光周期内就可以被加速至相对论能区；当激光功率密度达到$10^{22}\text{W}/\text{cm}^2$, 电子的动力学由辐射反作用主导；
...
- 本文我们回顾了经典电动力学中的辐射以及辐射反作用。

  经典电动力学中的辐射来自有源Maxwell方程组，辐射场对应解中正比于$1/R$的项；由电磁场中的辐射场部分计算Poynting矢量，可得辐射功率角分布及总功率；针对几种常见的运动电荷速度-加速度情形，给出了具体辐射功率公式。

  上述对辐射的处理并没有考虑辐射对运动电荷的反作用，原因是相比于电子由于加速度而改变的动能，辐射损失的能量实在太小，只有在长时间积累下才变得可观。此时可以通过Abraham-Lorentz公式进行计算，该公式能够满足能量守恒。

- 通过梳理本文，我解除了如下误解：

  迷思：Maxwell的电磁理论无法处理电子的运动（包含辐射反作用时），是因为理论本身不自洽。

  答案：而是当试图通过先求解电子所产生的电磁场并将其代回电子的运动方程，从而获得一个仅包含粒子自由度的封闭动力学方程时，电磁场的无限自由度被消除，其动力学效应被编码进粒子自身的运动方程之中，结果导致高阶时间导数的出现，并引入非局域性与因果性方面的困难。

  迷思：电子一直被认为是点粒子

  答案：为了解决自能发散、辐射反作用等问题，20世纪很多物理学家（Lorentz, Dirac, Rohrlich）严肃地考虑了电子的空间结构。

- 下一步：

  - Abraham-Lorentz-Dirac方程与Landau-Lifshitz方程
  - 量子电动力学中的辐射
  - 超强激光下的电子运动
  - 匀加速运动的电荷与等效原理
  - ...
...
[^6]: Schwinger, J. [Electromagnetic mass revisited](https://link.springer.com/article/10.1007/BF01906185). *Found Phys* **13**, 373–383 (1983).
